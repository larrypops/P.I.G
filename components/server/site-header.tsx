    ...
    <header className="sticky top-0 z-50 border-b border-white/60 bg-[color:rgba(249,245,236,0.82)] backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold uppercase tracking-[0.26em] text-[var(--brand)]">P.I.G</div>
        ...
        <nav className="hidden items-center gap-2 md:flex">
          {navigation.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-4 py-2 text-sm font-medium text-[var(--muted)] transition hover:bg-white hover:text-[var(--foreground)]"
            >
              {item.label}
            </Link>
          ))}
